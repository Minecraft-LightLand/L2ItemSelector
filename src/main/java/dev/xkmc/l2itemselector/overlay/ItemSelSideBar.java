...
public abstract class ItemSelSideBar<S extends SideBar.Signature<S>> extends SelectionSideBar<ItemStack, S> {
	...
	protected void renderEntry(Context ctx, ItemStack stack, int i, int selected) {
		boolean shift = L2Keys.hasShiftDown();
		int y = 18 * i + ctx.y0();
		...
		if (selected == i) {
			if (!stack.isEmpty() && ease_time == max_ease) {
				...
				GuiHelper.tooltip(ctx.g(), List.of(stack.getHoverName()), 0, 0);
	...
	public void renderSelection(GuiGraphicsExtractor g, int x, int y, int a, boolean available, boolean selected) {
		if (available) {
